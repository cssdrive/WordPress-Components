<article id="post-<?php the_ID(); ?>" <?php post_class('uk-article uk-margin-large-bottom'); ?>>
	...
	<header class="entry-header uk-margin">
		...
		<h1 class="entry-title uk-article-title"><span class="uk-badge"><?php printf( esc_html_e( 'Image', 'monopixel' ) ); ?></span> <?php the_title( sprintf( '<a class="uk-link-reset" href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a>' ); ?></h1>
